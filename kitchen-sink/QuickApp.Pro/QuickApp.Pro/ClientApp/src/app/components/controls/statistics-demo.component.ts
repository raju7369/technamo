...
export class StatisticsDemoComponent implements OnInit, OnDestroy {
  ...
  constructor(private alertService: AlertService, private mediaQuery: ObservableMedia) {
  }

  ngOnInit() {
    this.timerReference = setInterval(() => this.randomize(), 5000);

    this.mediaQuerySub = this.mediaQuery.asObservable().subscribe(change => this.chartLegend = change.mqAlias == "xs" ? false : true);
  }
